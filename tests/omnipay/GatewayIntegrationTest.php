...
class GatewayIntegrationTest extends GatewayTestCase
{
    ...
    public function testEnrollmentSuccess()
    {
        ...
        $this->assertEquals('30c061a8-862f-11e2-bd92-00163e6e1b2f', $response->getTransactionReference());
        $this->assertEquals('3d-acquirer:The resource was successfully created.', $response->getMessage());
    }
    ...
    public function testEnrollmentLiabilityShift()
    {
        $this->setMockHttpResponse('EnrollmentLiabilityShift.txt');
        $response = $this->gateway->enrollmentCheck($this->paymentOptions)->send();
        $this->assertTrue($response->isSuccessful());
        $this->assertFalse($response->isRedirect());
        $this->assertSame('Cardholder not participating.', $response->getMessage());
